...
def analyze_attack(username, password, attack_type):
    ...
    if "SQL Injection" in attack_type:
        ...
        if "or" in data and "=" in data:
            analysis["goal"] = "Bypass Authentication"
            analysis["technique"] = "OR-based injection"

        elif "union" in data:
            analysis["goal"] = "Extract Data"
            analysis["technique"] = "UNION SELECT"

        elif "sleep" in data:
            analysis["goal"] = "Blind Injection"
            analysis["technique"] = "Time-based attack"

    elif "XSS" in attack_type:
        analysis["risk"] = "High"

        if "<script>" in data:
            analysis["goal"] = "Execute JavaScript"
            analysis["technique"] = "Script Injection"

        elif "onerror" in data:
            analysis["goal"] = "Steal Data / Cookies"
            analysis["technique"] = "Event-based XSS"
...
def generate_suggestions(attack_type):

    if "SQL Injection" in attack_type:
        return [
            "Use parameterized queries",
            "Avoid f-strings in SQL queries",
...
def create_alert(cursor, username, ip, attack_type, analysis):

    suggestions = generate_suggestions(attack_type)
...
def home():
    if request.method == "POST":
        ...
        if attack_type != "Normal":
            analysis = analyze_attack(username, password, attack_type)
            create_alert(cursor, username, ip, attack_type, analysis)
